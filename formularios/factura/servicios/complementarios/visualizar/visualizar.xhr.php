...
$factura = $fscf->consultar($v->recibir("factura"));
...
for ($i = 2; $i > 0; $i--) {
  ...
  if($i==2){
  $pdf->SetXY($ml+57,30);$pdf->Write(0," - ORIGINAL - ");
  }else{
   $pdf->SetXY($ml+60,30);$pdf->Write(0," - COPIA - "); 
  }
  $pdf->SetXY($ml, 42.5);
...
//$pdf->Write(0, strtoupper($factura["vencimiento"]));
  ...
  while ($fila = $db->sql_fetchrow($consulta)) {
    ...
    $pdf->SetXY($ml + 20, $yp);$pdf->Write(0, strtoupper($fila["detalles"]));
    $pdf->SetXY($ml + 88, $yp);$pdf->Cell(50, 0, ($neto), 0, 0, 'R');
    $yp+=$ypi;
  }
  $db->sql_close();
  ...
  $factura["observacion"] = urldecode($factura["observacion"]);
  
  $pdf->SetXY($ml + 88, 154);$pdf->Cell(50, 0, $factura["iva"], 0, 0, 'R');
  $pdf->SetXY($ml + 88, 161);$pdf->Cell(50, 0, $factura["total"], 0, 0, 'R');
  $pdf->SetFont('Helvetica', 'B', 8);
  if(!empty($factura["observacion"])){
    $pdf->SetXY($ml+20,123);$pdf->Write(0,utf8_decode("OBSERVACIÓN:"));
    $obs=iconv('UTF-8', 'windows-1252', html_entity_decode($factura["observacion"]));
    $obs=strip_tags($obs);
    //$pdf->SetXY($ml+20,125);$pdf->MultiCell(70,3.5,strip_tags((utf8_decode($factura["observacion"]))));
    $pdf->SetXY($ml+20,125);$pdf->MultiCell(70,3.5,$obs);
  }
  $pdf->SetFont('Helvetica', 'B', 11);
  /** QR * */
